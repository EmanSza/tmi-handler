...
module.exports = class TwitchWrapper {
        ...
        return new Promise((resolve, reject)=>{
            ...
            for (const file of files){
                if (file.isDirectory()) {
                    ...
                    command.length ? command.forEach((cmd)=>{
    ...
    loadEvents(eventPath = '/commands') {
        ...
        return new Promise((resolve, reject)=>{
            ...
            let files = fs.readdirSync(path.resolve(process.cwd() + eventPath), {
                withFileTypes: true
            ...
            for (const file1 of files){
                if (file1.isFile()) {
                    let eventFile = require(path.resolve(process.cwd() + eventPath + '/' + file1.name));
                    const eventObj = {
    ...
    message({ channel , userstate , message , self , client  }) {
        ...
        if (message[0] === this.prefix) {
            ...
            let command1 = message.substring(1).toLowerCase();
            // Split the message into an array
            let commandSplit = command1.split(' ');
    ...
    constructor(options){
        ...
        if (this.commandPath != undefined) throw new Error('commadPath is deprecated from the options object please use loadCommands');
        // if the user is using eventPath, throw an error
        if (this.eventPath != undefined) throw new Error('eventPath is deprecated from the options object please use loadEvents');
